const gradientMapping = {
  ...
  const GlassIcons = ({ items, className }) => {
    ...
    return (
      <div
        className={`grid p-5 gap-[2em] grid-cols-2  mx-auto py-[2em] overflow-visible ${className || ""
          }`}
      >
        {items.map((item, index) => (
          ...
          <button
            ...
            className={`relative hover:cursor-pointer bg-transparent outline-none w-[4.5em] h-[4.5em] [perspective:24em] [transform-style:preserve-3d] [-webkit-tap-highlight-color:transparent] group ${item.customClass || ""
              }`}
          >
            <span
            ...
            >
              <span className="m-auto w-[1.7em] h-[1.7em] flex items-center justify-center" aria-hidden="true">
                {item.icon}
              </span>
            </span>
